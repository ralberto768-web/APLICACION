...
def accion():from __future__ import annotations
...
@app.route("/audio/<int:audio_id>", methods=["GET"])
def audio(audio_id: int):
    ruta = ruta_audio(audio_id)
    if not ruta.exists() or ruta.suffix.lower() != ".wav":
        abort(404)
    return send_file(Path(ruta), mimetype="audio/wav", download_name=f"audio{audio_id}.wav")
